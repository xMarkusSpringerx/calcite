...
}
{
    <INTERVAL> { s = span(); } intervalQualifier = IntervalQualifier() {
        return new SqlDataTypeSpec(new SqlIntervalTypeNameSpec(intervalQualifier, s.end(this)), s.pos());
    }
}
...
}
{
    ...
    (
        <LOGGING> { return new SqlAlterTable(s.end(this), ifExists, id, true); }
    |
        <NOLOGGING>  { return new SqlAlterTable(s.end(this), ifExists, id, false); }
    |
        <ADD> [<COLUMN>] colIgnoreErr = IfNotExistsOpt() cols = ColumnWithTypeOrList() {
            return new SqlAlterTableAddColumn(s.end(this), ifExists, id, colIgnoreErr, cols);
        }
    |
        <DROP> [<COLUMN>] colIgnoreErr = IfExistsOpt() cols = SimpleIdentifierOrList() {
            return new SqlAlterTableDropColumn(s.end(this), ifExists, id, colIgnoreErr, cols);
        }
    |
    <RENAME> [<COLUMN>] colIgnoreErr = IfExistsOpt() cols = SimpleIdentifierOrList() {
            return new SqlAlterTableRenameColumn(s.end(this), ifExists, id, colIgnoreErr, cols);
            }
    )
}
