...
import org.apache.calcite.sql.parser.ddl.CeloSqlParserImpl;
...
class ServerParserTest extends SqlParserTest {
  ...
  @Override public SqlParserFixture fixture() {
    return super.fixture()
        .withConfig(c -> c.withParserFactory(CeloSqlParserImpl.FACTORY));
  }
  ...
  @Test void testAlterTableAddColumn() {
    final String sql = "alter table x add column y int";
    final String expected = "ALTER TABLE `X` ADD COLUMN `Y` INTEGER";
    sql(sql).ok(expected);
  }

  @Test void testAlterTableDropColumn() {
    final String sql = "alter table x drop column y";
    final String expected = "ALTER TABLE `X` DROP COLUMN `Y`";
    sql(sql).ok(expected);
  }

  @Test void testAlterTableRenameColumn() {
    final String sql = "alter table x rename column y";
    final String expected = "ALTER TABLE `X` RENAME COLUMN `Y`";
    sql(sql).ok(expected);
  }

  @Test void testAlterTableModifyColumn() {
    final String sql = "alter table x modify column y varchar(255)";
    final String expected = "ALTER TABLE `X` MODIFY COLUMN `Y` VARCHAR(255)";
    sql(sql).ok(expected);
  }
